...
namespace TexDrawLib.Core
{
    public class WordAtom : LayoutAtom
    {
        ...
        {
            var hyp = TEXPreference.main.GetHyphenationEngine();
            if (hyp == null)
            {
                return;
            }
            var str = new List<string>();
            foreach (Atom item in children)
            {
                if (item is SymbolAtom sitem && sitem.metadata.symbol.Length <= 3)
                {
                    str.Add(sitem.metadata.symbol);
                }
                else if (item is CharAtom citem)
                {
                    str.Add(citem.character + "");
                }
                else
                {
                    str.Add("");
            ...
            var sstr = string.Join("", str);
            var ssstr = hyp.HyphenateText(sstr);
            if (ssstr == sstr)
            {
                return;
            }
            var k = 0; // pos of str
            var l = 0; // pos of inner str
            for (int i = 0; i < ssstr.Length; i++)
            {
                if (ssstr[i] == '-')
                {
                    if (str[k].Length == l + 1)
                    {
                        var hype = HyphenBreak.Get();
                        hype.Copy(children[k] as CharAtom, false);
                        hype.character = '-';
                        if (hype.fontIndex >= 0)
                        {
                            children.Insert(k + 1, hype);
                            hasHyphen = true;
                        }
                        else
                        {
