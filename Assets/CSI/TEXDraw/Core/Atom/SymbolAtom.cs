...
namespace TexDrawLib.Core
{
    public class AutoDelimitedGroupAtom : InlineAtom
    {
        ...
        public override Box CreateBox(TexBoxingState state)
        {
            ...
            var leftBox = left?.CreateBoxMinHeight(height, exactFit);
            var rightBox = right?.CreateBoxMinHeight(height, exactFit);
            if (leftBox != null)
            {
        ...
        public override void ProcessParameters(string command, TexParserState state, string value, ref int position)
        {
            ...
            exactFit = state.Math.delimiterExactFit;
    ...
    public class SymbolAtom : CharAtom
    {
        ...
        public Box CreateBoxMinHeight(float minimum, bool exactFit = false)
        {
            Box prevBox = null;
            var metadata = this.metadata;
            ...
            if (box.TotalHeight < minimum && metadata.extension.enabled)
            {
                ...
                if (container.TotalHeight < minimum && ext.repeat.Has)
                {
                    ...
                    float offset = (container.TotalHeight - minimum);
                    repeatBox.depth -= offset / loop;
            ...
            if (prevBox != null && exactFit)
            {
                float ratio = UnityEngine.Mathf.InverseLerp(prevBox.TotalHeight, box.TotalHeight, minimum);
                float geomRatio = minimum / box.TotalHeight;
                if (geomRatio < 1)
                {
                    box.height = UnityEngine.Mathf.Lerp(prevBox.height, box.height, ratio);
                    box.depth = UnityEngine.Mathf.Lerp(prevBox.depth, box.depth, ratio);
                    box.width = UnityEngine.Mathf.Lerp(prevBox.width, box.width, ratio);
                    if (box is VerticalBox vb)
                    {
                        foreach (var vbi in vb.children)
                        {
                            vbi.height *= geomRatio;
                            vbi.depth *= geomRatio;
                            vbi.width *= geomRatio;
                        }
                    }
